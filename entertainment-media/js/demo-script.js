var mep_plugin_array = [
  ...
  },
  {
    ...
    'source'           : 'https://downloads.wordpress.org/plugin/google-language-translator.zip',
    'required'         : true,
    'force_activation' : false,
    'text-domain'      : 'google-language-translator',
    'file_name'        : 'google-language-translator.php'
  },
  {
...
jQuery('#demo-importer-form').on('submit', async function (e) {
  ...
  if(confirm("Do you really want to do this?")){
    ...
    mep_plugin_array.map(function (plugin_url_data, index) {
      ...
      console.log('onclick');
      jQuery.ajax({
        url: entertainment_media_demo_object.ajax_url,
        type: 'post',
        data: data_to_post,
        async: false
